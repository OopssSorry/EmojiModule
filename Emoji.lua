-- Original: https://github.com/kitsunies/emoji.lua
...
function EmojiLib.get(query)
    return emojis.list[query]
end

function EmojiLib.which(query)
    return emojis.reversed[query]
end

function EmojiLib.emojify(query, missing, format)
...
function EmojiLib.unemojify(query)
    for k, v in pairs(emojis.list) do
...
function EmojiLib.replace(query, fn)
	fn = fn or keyop
	assert(type(fn(toTable())) == 'string', 'callback function does not return a string')
	for key, emoji in pairs(emojis.list) do
		query = query:gsub(emoji, fn(toTable(emoji, key)))
	end
	return query
end

function EmojiLib:EmojiReplace(str)
	str = tostring(str)
	if not str:find(":") then
		return str
	else
		for key, emoji in pairs(emojis.list) do
			str = str:gsub(":"..key..":",emoji)
		end
		return str
	end 
end
